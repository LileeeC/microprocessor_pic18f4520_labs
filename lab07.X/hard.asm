...
ISR:				; Interrupt發生時，會跳到這裡執行。
    ...
    XORWF STA, F		; 0 <-> 1
    MOVF STA, W
    BNZ CNTodd			; check state flag, if != 0 (==1, count odd) then reset counter to 1
    BRA CNTeven
CNTeven:
    CLRF CNT			; will be count even
    BRA Show
CNTodd:
    MOVLW 0x01
    MOVWF CNT			; will be count pdd
Show:
    MOVF CNT, W
...
main:
    ...
    MOVF STA, W
    BNZ State2			; check state flag, if != 0 (==1) then do count dowm
    
State1:
    DELAY d'111', d'63'		; 0.25s
    MOVLW 2
    ADDWF CNT, F		; counter += 2
    MOVF CNT, W
    ANDLW 0x0F		; only keep low 4 bit (b'00001111')
    MOVWF CNT
