...
ISR:				
    ...
    MOVF SKIP, W	    ; if skip_flag == 1, then go to Skip 
    BNZ Skip
    
    ; done delay
    DECF CNT, F
    MOVF CNT, W
    ANDLW 0x0F	    ; use mask to cycle through 0x00 -> 0x0F automatically
    MOVWF CNT
    
    SWAPF WREG, W	    ; counter value is already in WREG
    MOVWF LATD
    
    MOVF DELAY1
    BNZ Delay0.5	    ; if delay_flag == 1 then next round will be "delay 0.5s"
    
Delay1:
    BSF DELAY1, 0	    ; next round delay 1s
    BSF SKIP, 0		    ; mext round skip the 0.5s tick
    BRA EndISR
Delay0.5:
...
Initial:			
    ...
    MOVLW 0x0F
    MOVWF CNT			; counter initial 15
    CLRF DELAY1			; first delay = 0.5s
    CLRF SKIP
    
    MOVF CNT, W
    SWAPF WREG, W		; b'00001111' -> b'11110000' for output at RD7~RD4
    MOVWF LATD
